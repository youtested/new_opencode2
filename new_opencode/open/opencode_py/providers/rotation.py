...
class Rotation:
    ...
    def __init__(self, lanes: list[dict[str, str]], make_provider: Callable[[str, str], Any], session_id: str | None = None):
        ...
        try:
            from .zen import _coerce_session_id as _coerce
            self.session_id = _coerce(session_id)
        except Exception:
            ...
            try:
                ...
                if e.retryable:
                    if index == 0:
                        ...
                        try:
                            from .hints import hint_block as _hint
                            _h = _hint(getattr(e, "status", None), getattr(e, "message", None) or str(e))
                        except Exception:
                            _h = ""
                        suffix = (
                            f"\n\n{_h}"
                        ...
                        raise ProviderError(f"{provider_id}: {e}{suffix}", retryable=True)
...
def _probe_headers(session: str | None = None) -> dict:
    ...
    try:
        from .zen import _coerce_session_id as _coerce
        from .zen import _new_request_id as _new_req
        from .zen import _user_agent as _ua
    except Exception:
        _ua = lambda: "opencode/latest/1.18.31/cli"  # noqa: E731
        _coerce = lambda s: s or "health"  # noqa: E731
        _new_req = lambda: "msg_test"  # noqa: E731
    sid = _coerce(session or "health-probe")
    # _coerce mints a RANDOM valid id each call — for probes we need a
    # STABLE per-model id, so derive deterministically from the name when
    # the caller passed an invalid placeholder like "health-<model>".
    if (session or "").startswith("health-") and sid != session:
        import hashlib as _hl
        import re as _re
        h = _hl.sha256(session.encode()).hexdigest()[:12].lower()
        tail = _re.sub(r"[^0-9A-Za-z]", "x", session)[-14:].ljust(14, "x")
        sid = f"ses_{h}{tail[:14]}"
    headers = {
        "User-Agent": _ua(),
...
def _probe_zen_model(model_id: str) -> bool:
    ...
    Automatic for future models: order comes from the live catalog npm hint
    (anthropic -> messages first) then the cached winner, then every Zen
    transport (responses/chat/anthropic). 200/429 = alive; anything else =
    try next transport. Transport errors propagate so the health layer can
    ...
    def _hint(mid: str) -> str:
        try:
            cat = (fetch_catalog().get("opencode") or {}).get("models", {})
            bare = str(mid).split("/", 1)[-1]
            info = cat.get(bare) or cat.get(str(mid)) or {}
            npm = str(((info.get("provider") or {}).get("npm")) or "").lower()
            if "anthropic" in npm:
                return "anthropic"
        except Exception:
            pass
    ...
    hint = _hint(model_id)
    order: list[str] = []
    for cand in ([hint] if hint else []) + ([pref] if pref else []) + ["responses", "chat", "anthropic"]:
    ...
    for ep in order:
        try:
            if ep == "responses":
                ...
                ok = _probe_anthropic_model(model_id)
...
def _probe_anthropic_model(model_id: str) -> bool:
    """Anthropic Messages probe (one leg of _probe_zen_model)."""
    try:
        import httpx

        resp = httpx.post(
            f"{ZEN_CHAT_URL}/messages",
