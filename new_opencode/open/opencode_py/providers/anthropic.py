    ...
    def _check_status(self, resp: "httpx.Response") -> None:
        ...
        if resp.status_code == 429:
            ...
            try:
                ...
                _h = _hint(429, body, getattr(self, "model", ""))
            except Exception:
                _h = ""
            msg = f"{self.name} rate limited (429): {body}"
            if _h:
                msg += f"\n\n{_h}"
            raise RateLimitError(msg, retry_after=ra)
        if resp.status_code >= 500:
            try:
                from .hints import hint_block as _hint
                _h = _hint(resp.status_code, body, getattr(self, "model", ""))
            except Exception:
                _h = ""
            msg = f"{self.name} server error ({resp.status_code}): {body}"
            if _h:
                msg += f"\n\n{_h}"
        ...
        try:
            ...
            _h = ""
        msg = f"{self.name} error ({resp.status_code}): {body}"
        if _h:
            msg += f"\n\n{_h}"
