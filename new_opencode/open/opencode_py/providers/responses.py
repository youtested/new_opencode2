...
def get_preferred_endpoint(model_id: str) -> str:
    """Cached working transport: ``"responses"`` | ``"chat"`` | ``"anthropic"``.
    ...
    try:
        ...
        if isinstance(ep, dict):
            ...
            try:
                ...
                fresh = False
            if fresh and ep.get("endpoint") in ("responses", "chat", "anthropic"):
                return ep["endpoint"]
        elif ep in ("responses", "chat", "anthropic"):
            return ep
    except Exception:
        pass
    return "responses"
...
def set_preferred_endpoint(model_id: str, endpoint: str) -> None:
    ...
    if endpoint not in ("responses", "chat", "anthropic"):
        return
    try:
        import json as _json
        import time as _time
...
def _check_responses_status(resp: "httpx.Response", name: str) -> None:
    ...
    if resp.status_code >= 500:
        try:
            ...
            _h = _hint(resp.status_code, body)
        except Exception:
            _h = ""
        msg = f"{name} server error ({resp.status_code}): {body}"
        if _h:
            msg += f"\n\n{_h}"
        raise ProviderError(msg, retryable=True, status=resp.status_code)
    ...
    try:
        ...
        _h = ""
    msg = f"{name} error ({resp.status_code}): {body}"
    if _h:
        msg += f"\n\n{_h}"
    raise ProviderError(msg, status=resp.status_code)
